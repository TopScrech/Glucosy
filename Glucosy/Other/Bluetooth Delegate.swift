...
final class BluetoothDelegate: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate, Logging {
    ...
    func centralManager(_ manager: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData advertisement: [String: Any], rssi: NSNumber) {
        ...
        if let manufacturerData {
            ...
            if companyId >= BLE.companies.count {
                companyId = BLE.companies.count - 1
            }    // when 0xFFFF
        }
    ...
    func centralManager(_ manager: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ...
        if error != nil {
            ...
            if app.transmitter != nil && (settings.preferredTransmitter == .none || settings.preferredTransmitter.id == app.transmitter.type.id) {
                ...
                if errorCode == .connectionTimeout {
                    main.errorStatus("Connection timed out. Waiting...")
                }
                ...
                if app.transmitter.type == .transmitter(.dexcom) {
                    ...
                    main.status("Scanning for Dexcom...") //  allow stopping from Console
