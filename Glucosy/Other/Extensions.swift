...
extension Data {
    ...
    func hexDump(header: String = "", address: Int = -1, startBlock: Int = -1, escaping: Bool = false) -> String {
        ...
        while offset < endIndex {
            ...
            if address != -1 {
                str += (address + offset).hex + " "
            }
            
            if startBlock != -1 {
                str += "#\((startBlock + offset / 8).hex) "
            }
            
            if address != -1 || startBlock != -1 {
                str += " "
            }
...
extension UInt16 {
    ...
    var data: Data {
        Data([UInt8(self & 0xFF), UInt8(self >> 8)])
    }
}
...
extension UInt32 {
    ...
    var data: Data {
        Data([UInt8(self & 0xFF), UInt8(self >> 8 & 0xFF), UInt8(self >> 16 & 0xFF), UInt8(self >> 24)])
    }
    
    var formattedInterval: String {
        Double(self).formattedInterval
    }
}
...
extension Int {
    ...
    var formattedInterval: String {
        Double(self * 60).formattedInterval
    }
