...
extension Error {
    ...
    var iso15693Description: String {
        IS015693Error(rawValue: self.iso15693Code)?.description ?? "[code: 0x\(self.iso15693Code.hex)]"
    }
}
...
class NFC: NSObject, NFCTagReaderSessionDelegate, Logging {
    ...
    func readRaw(_ address: Int, _ bytes: Int) async throws -> (Int, Data) {
        ...
        while remainingBytes > 0 && retry <= retries {
            ...
            if buffer.count == 0 {
                debugLog("NFC: sending '\(readRawCommand.code.hex) \(readRawCommand.parameters.hex)' custom command (\(sensor.type) read raw)")
            }
    ...
    func testNFCCommands() async {
        ...
        if settings.userLevel > .basic {
            ...
            var commands: [NFCCommand] = [
                sensor.nfcCommand(.readAttribute),
